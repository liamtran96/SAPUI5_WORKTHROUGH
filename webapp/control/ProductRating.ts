...
import RatingIndicator, {
  RatingIndicator$LiveChangeEvent,
...
export default class ProductRating extends Control {
  ...
  reset(): void {
    ...
    (this.getAggregation("_label") as Label).setText(
      resourceBundle.getText("productRatingLabelInitial"),
    );
  ...
  _onRate(event: RatingIndicator$LiveChangeEvent): void {
    ...
    (this.getAggregation("_label") as Label).setText(
      resourceBundle.getText("productRatingLabelIndicator", [
        value,
        (event.getSource() as RatingIndicator).getMaxValue(),
      ]),
    );
  ...
  _onSubmit(event: Button$PressEvent): void {
    ...
    (this.getAggregation("_label") as Label).setText(
      resourceBundle.getText("productRatingLabelFinal"),
    );
